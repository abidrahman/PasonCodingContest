...
public class Tank {
            ...
            for (int i = 0; i < players.length(); i++) {
                ...
                if (players.getJSONObject(i).getString("name").equals(gameInfo.getTeamName())) {
                    ...
                    for (int j = 0; j < our_tanks.length(); j++) {
                        ...
                        JSONArray tank_coordinate = tank.getJSONArray("position");
    ...
    public ArrayList<String> strategy() throws JSONException {
        ...
        double distance;
        boolean found_enemy = find_enemy();
    ...
    private boolean find_enemy() throws JSONException{
        //Find closest enemy
        for (Vector e : enemy_tank_coordinates) {
            distance = distance(e, this_tank.position);
            if ((distance < closest_distance) && !doesCollide(this_tank.position, e) && distance < 100) {
                enemy.x = e.x;
                enemy.y = e.y;
    ...
    private double find_closest_enemy_angle() throws JSONException {
    ...
    private ArrayList<String> aim() throws JSONException {
        ...
        double closest_enemy_angle = find_closest_enemy_angle();
        if (closest_enemy_angle < Math.PI && closest_enemy_angle > 0) {
            String rotate_command = command.rotateTurret(tankID, CW, closest_enemy_angle, gameInfo.getClientToken());
            commands.add(rotate_command);
        } else if (closest_enemy_angle >= Math.PI && closest_enemy_angle < 2*Math.PI) {
            String rotate_command = command.rotateTurret(tankID, CCW, 2*Math.PI - closest_enemy_angle, gameInfo.getClientToken());
            commands.add(rotate_command);
        }
    ...
    private ArrayList<String> fire() throws JSONException {
        ...
        double closest_enemy_angle = find_closest_enemy_angle();
        if ((closest_enemy_angle <= 0.02 || closest_enemy_angle >= (2*Math.PI - 0.02)) && (closest_distance <= 100) && (!friendly_in_the_way(closest_enemy_angle))) {
            String fire_command = command.fire(tankID, gameInfo.getClientToken());
            commands.add(fire_command);
        }

        if (friendly_in_the_way(closest_enemy_angle)) {
